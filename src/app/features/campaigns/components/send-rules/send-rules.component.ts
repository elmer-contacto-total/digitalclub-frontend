...
import { CampaignService, BulkSendRules } from '../../../../core/services/campaign.service';
    ...
    <div class="send-rules-container">
      ...
      @if (isLoading()) {
        ...
        <div class="rules-card">
          ...
          <!-- Delay Settings -->
          <div class="section">
            <h3><i class="ph-clock"></i> Delays entre Mensajes (Electron)</h3>
            <div class="form-row">
              <div class="form-group">
          ...
          <!-- Cloud API -->
          <div class="section">
            <h3><i class="ph-cloud"></i> Cloud API</h3>
            <div class="form-group">
              <label for="cloudDelay">Delay entre envíos Cloud API (ms)</label>
              <input type="number" id="cloudDelay" [(ngModel)]="formData.cloud_api_delay_ms"
                     class="form-control" min="50" max="5000">
              <span class="help-text">Milisegundos entre cada envío via Cloud API</span>
            </div>
          </div>
...
export class SendRulesComponent implements OnInit, OnDestroy {
  ...
  private loadRules(): void {
    ...
    this.campaignService.getRules().pipe(
      ...
      next: (response) => {
        ...
        this.formData = {
          ...
          cloud_api_delay_ms: r.cloud_api_delay_ms,
          enabled: r.enabled
        };
  ...
  save(): void {
    ...
    this.campaignService.updateRules(this.formData).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
      next: () => {
