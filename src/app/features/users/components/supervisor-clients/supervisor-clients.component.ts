 ...
 * PARIDAD RAILS: app/views/admin/users/supervisor_clients.html.erb
...
import { UserService, PaginationParams, SupervisorClientsParams } from '../../../../core/services/user.service';
...
@Component({
      ...
      <div class="clients-sidebar">
        ...
        @if (activeOnly()) {
          <div class="filters-container">
            <!-- Agent Filter (only for supervisor view) -->
                    ...
                    >
                      <td class="col-nombre">
                        <div class="nombre-cell">
                          <span class="nombre-text">{{ client.fullName || getFullName(client) }}</span>
  ...
  `,
  styleUrls: ['./supervisor-clients.component.scss']
})
export class SupervisorClientsComponent implements OnInit, OnDestroy {
  ...
  // Filters
  agentFilter: string = 'all';  // For supervisor: filter by specific agent
  ...
  ngOnInit(): void {
    ...
    // Load subordinate agents for filter
    this.loadAgents();
  ...
  getFullName(client: UserListItem): string {
  ...
  private loadClients(refresh = false): void {
    ...
    if (this.agentFilter !== 'all') {
      params.managerId = parseInt(this.agentFilter, 10);
    }
    if (this.ticketFilter !== 'all') {
      params.ticketStatus = this.ticketFilter;
    }
    if (this.messageFilter !== 'all') {
      params.messageStatus = this.messageFilter;
    }
