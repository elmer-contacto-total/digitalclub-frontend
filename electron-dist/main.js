...
const media_security_1 = require("./media-security");
// Fingerprint único para esta instalación
let userFingerprint;
let mainWindow = null;
let whatsappView = null;
let lastClipboard = '';
let lastScannedMessages = new Set();
 ...
 */
async function sendMediaToServer(payload) {
    try {
...
function handleMediaCaptured(data) {
    ...
    const payload = {
        mediaId: (0, media_security_1.generateMediaId)(),
...
function createWindow() {
    ...
    // TEMPORAL: Abrir DevTools para depurar pantalla gris
    // Descomentar la siguiente línea si necesitas depurar:
...
function createWhatsAppView() {
    if (!mainWindow || whatsappView)
        return;
    console.log('[HablaPe] Creando WhatsApp BrowserView...');
    // Usar partición persistente para guardar sesión
    const whatsappSession = electron_1.session.fromPartition('persist:whatsapp');
    ...
    whatsappView = new electron_1.BrowserView({
        webPreferences: {
            nodeIntegration: false,
    ...
    (0, media_security_1.initializeMediaSecurity)(whatsappView, mainWindow, userFingerprint.odaId, {
    ...
    const evasionScript = (0, fingerprint_generator_1.generateEvasionScript)(userFingerprint);
...
function updateWhatsAppViewBounds() {
    if (!mainWindow || !whatsappView || !whatsappVisible)
        return;
...
function animateWhatsAppViewBounds() {
    if (!mainWindow || !whatsappView || !whatsappVisible)
        return;
...
async function scanChatMessages(telefono) {
    if (!whatsappView || !mainWindow || !whatsappVisible)
        return [];
    try {
...
function clearScannedMessages() {
    lastScannedMessages.clear();
}
// Sistema Anti-Ban: Detección de clipboard
function startClipboardMonitor() {
    setInterval(() => {
        const currentClipboard = electron_1.clipboard.readText().trim();
...
function registerShortcuts() {
    ...
    electron_1.globalShortcut.register('CommandOrControl+Shift+C', () => {
        const phone = electron_1.clipboard.readText().trim();
    ...
    electron_1.globalShortcut.register('CommandOrControl+Shift+T', () => {
        if (mainWindow) {
...
function setupIPC() {
    ...
    electron_1.ipcMain.handle('show-whatsapp', () => {
        showWhatsAppView();
        return whatsappVisible;
    });
    electron_1.ipcMain.handle('hide-whatsapp', () => {
        hideWhatsAppView();
        return !whatsappVisible;
    });
    electron_1.ipcMain.handle('get-whatsapp-visible', () => {
        return whatsappVisible;
    });
    // Toggle sidebar
    electron_1.ipcMain.on('toggle-sidebar', () => {
    ...
    electron_1.ipcMain.on('update-layout', (_, data) => {
    ...
    electron_1.ipcMain.on('sidebar-toggle', (_, collapsed) => {
    ...
    electron_1.ipcMain.on('set-view', (_, view) => {
        // Ya no hace nada - el control es explícito con show/hide
    });
    // Obtener estado de sesión WhatsApp
    electron_1.ipcMain.handle('get-whatsapp-status', async () => {
    ...
    electron_1.ipcMain.handle('scan-messages', async (_, telefono) => {
    ...
    electron_1.ipcMain.handle('scan-new-messages', async (_, telefono) => {
        const messages = await scanChatMessages(telefono);
        return messages;
    });
    // Obtener ancho disponible para Angular
    electron_1.ipcMain.handle('get-angular-bounds', () => {
        if (!mainWindow)
            return null;
    ...
    electron_1.ipcMain.handle('clear-session-and-reload', async () => {
    ...
    electron_1.ipcMain.handle('full-reset', async () => {
...
electron_1.app.setPath('userData', path.join(electron_1.app.getPath('appData'), 'HablaPe'));
...
electron_1.app.whenReady().then(async () => {
    ...
    userFingerprint = (0, fingerprint_generator_1.getOrCreateFingerprint)();
